...
// Compute a smart back label from the previous entry in the history stack
  ...
  // entry; the original note is left untouched. Personal/business defaults to
  // the current mode.
  const handleNoteToCalendar = (note) => setModal({
    type: 'add_diary',
    prefill: {
      text: note.text || '',
  ...
  const sendEmail = async ({ personId, recipients, subject, body, threadId, inReplyTo, attachmentFileIds }) => {
    const res = await data.email.send({ personId, recipients, subject, body, threadId, inReplyTo, attachmentFileIds });
